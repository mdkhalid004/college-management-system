...
public class DashboardServiceImpl implements DashboardService {
    ...
    public List<RecentActivityDto> getRecentActivities() {
        ...
        for (Student s : recentStudents) {
            activities.add(new RecentActivityDto(
                    "🧑‍🎓",
                    s.getFirstName() + " " + s.getLastName() + " enrolled in " + s.getDepartment().getDepartmentName(),
                    "Recently Added"
            ));
        }
    ...
    public List<UpcomingEventDto> getUpcomingEvents() {
        ...
        List<Event> events = eventRepository.findTop3ByEventDateAfterOrderByEventDateAsc(LocalDateTime.now());
        ...
        for (Event e : events) {
            ...
            String schedule = formattedDate + " @ " + e.getLocation();
